...
mod tests {
    ...
    fn check_escaped() -> Result<()> {
        {
            ...
            let pair = pairs
                .next()
                .ok_or_else(|| anyhow!("Expected a pair, but found none"))?;
            let char_pair = pair
                .into_inner()
                .next()
    ...
    fn check_mixed_styled() -> Result<()> {
        ...
        let bold_content = bold_inner.next().ok_or_else(|| {
            anyhow!("Expected a pair with content rule inside bold, but found none")
        })?;
    ...
    fn check_paragraph() -> Result<()> {
        ...
        let pair = pairs
            .next()
            .ok_or_else(|| anyhow!("Expected a paragraph, but found none"))?;
        ...
        let plain2 = line1_inner.next().ok_or_else(|| {
            anyhow!("Expected plain_text after bold in the first line, but found none")
        })?;
    ...
    fn check_code_block() -> Result<()> {
        ...
        let pair = pairs
            .next()
            .ok_or_else(|| anyhow!("Expected a code_block, but found none"))?;
    ...
    fn check_horizontal_rule() -> Result<()> {
        ...
        for input in inputs {
            ...
            let pair = pairs
                .next()
                .ok_or_else(|| anyhow!("Expected a horizontal_rule, but found none"))?;
    ...
    fn check_markdown() -> Result<()> {
        ...
        let pair = pairs
            .next()
            .ok_or_else(|| anyhow!("Expected a markdown root, but found none"))?;
        ...
        let bold_content = bold
            .into_inner()
            .next()
            .ok_or_else(|| anyhow!("Expected bold content text"))?;
